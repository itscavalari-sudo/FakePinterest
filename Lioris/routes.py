...
def homepage():
    ...
    if formlogin.validate_on_submit():
        ...
        if usuario and bcrypt.check_password_hash(usuario.senha, formlogin.senha.data):
            login_user(usuario)
            return redirect(url_for("perfil", usuario=usuario.username,id_usuario=usuario.id))
    return render_template("homepage.html", form=formlogin)

@app.route("/criarconta", methods=["GET", "POST"])
def criarconta():
    form_criarconta = FormCriarConta()
    print("SUBMIT", form_criarconta.is_submitted())
    print("VALIDOU", form_criarconta.validate_on_submit())
    print("Erros:", form_criarconta.errors)
